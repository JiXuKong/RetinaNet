...
ckecpoint_file = cfg.val_restore_list
...
def pred_model():
    ...
    for i in range(cfg.batch_size):
        ...
        nms_box, nms_score, nms_label = gpu_nms(pred_regress_box_i, pred_scores, cfg.class_num-1, 100, 0.1, 0.5)
...
def result(boxes, scores, label):
    ...
    for val_step in range(1, cfg.test_num+1):
        ...
        b, s, l = sess.run([boxes, scores, label], feed_dict = val_feed_dict)
    ...
    for classidx in range(1, cfg.class_num):#从1到21，对应[bg,...]21个类（除bg）
        rec, prec, ap = voc_eval(gt_dict, val_pred, classidx, iou_thres=0.4, use_07_metric=False)
