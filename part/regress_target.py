...
def regress_target_tf(gt_box, anchor_box):
    '''
    gt_box: [M, 4]
    x1, y1, x2, y2
    anchor_box:[M, 4]
    ...
    x1, y1, x2, y2 = x1/640.0, y1/640.0, x2/640.0, y2/640.0
    w = x2 - x1
    h = y2 - y1
    x_ctr = x1 + w/2
    y_ctr = y1 + h/2
    ...
    xx1, yy1, xx2, yy2 = xx1/640.0, yy1/640.0, xx2/640.0, yy2/640.0
    ww = xx2 - xx1
    hh = yy2 - yy1
    xx_ctr = xx1 + ww/2
    yy_ctr = yy1 + hh/2
...
def reverse_regress_target_tf(pred_box, anchor_box):
    '''
    anchor_box: [-1, 4]
    pred_box:[-1, 4]
    '''
...
#     return tf.concat([x1,y1,x2,y2], axis=-1)
    return tf.stop_gradient(tf.concat([x1,y1,x2,y2], axis=-1)*640.0)