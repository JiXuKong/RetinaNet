...
class Retinanet(object):
    def __init__(self, is_training):
        ...
        self.pi = cfg.pi
        self.image = tf.placeholder(tf.float32, shape = [cfg.batch_size, self.image_size, self.image_size, 3])
        self.label = tf.placeholder(tf.int32, shape = [None, None, 5])
        self.num_boxes = tf.placeholder(tf.int32, shape = [None])
    ...
    def upsample_layer(self, inputs, factor=2., scope):
        with tf.name_scope(scope):
            h, w = inputs.shape[1].value, inputs.shape[2].value
            new_height, new_width = int(h*factor), int(w*factor)

            channels = inputs.shape[3].value
            rate = 2
            x = tf.reshape(inputs, [cfg.batch_size, new_height//rate, 1, new_width//rate, 1, channels])
            x = tf.tile(x, [1, 1, rate, 1, rate, 1])
            x = tf.reshape(x, [cfg.batch_size, new_height, new_width, channels])
            return x
...
#         end_points = resnet_base(self.image, self.is_training, 'resnet_v1_50')
    ...
    def FPN_structure(self, projection_norm = True):
        ...
        with tf.variable_scope('FeatureExtractor/resnet_v1_50/bifpn'):
            with slim.arg_scope([slim.conv2d], weights_regularizer=slim.l2_regularizer(4e-3),
                ...
                p3, p4, p5 = end_points['p3'], end_points['p4'], end_points['p5']
                with tf.variable_scope("intermediate_p6"):
                    ...
                    temp = w1 * p6 + w2 * self.upsample_layer(p7, factor=2., 'p7_upsample')
                    P_6_td = conv_bn_relu(temp/(w1 + w2 + EPSILON), W_bifpn, "P_6_td", "P_6_td/BatchNorm")
                    
                with tf.variable_scope("intermediate_p5"):
                    w1 = tf.get_variable("W1", [1, 1, 1, 1], initializer=tf.initializers.he_normal())
                    w2 = tf.get_variable("W2", [1, 1, 1, 1], initializer=tf.initializers.he_normal())
                    temp = w1 * p5 + w2 * self.upsample_layer(P_6_td, factor=2., 'p6_td_upsample')
                    P_5_td =  conv_bn_relu(temp/(w1 + w2 + EPSILON), W_bifpn, "P_5_td", "P_5_td/BatchNorm")
                
                with tf.variable_scope("intermediate_p4"):
                    w1 = tf.get_variable("W1", [1, 1, 1, 1], initializer=tf.initializers.he_normal())
                    w2 = tf.get_variable("W2", [1, 1, 1, 1], initializer=tf.initializers.he_normal())
                    temp = w1 * p4 + w2 * self.upsample_layer(P_5_td, factor=2., 'p5_td_upsample')
                    P_4_td =  conv_bn_relu(temp/(w1 + w2 + EPSILON), W_bifpn, "P_4_td", "P_4_td/BatchNorm")
               
                with tf.variable_scope("output_p3"):
                    w1 = tf.get_variable("W1", [1, 1, 1, 1], initializer=tf.initializers.he_normal())
                    w2 = tf.get_variable("W2", [1, 1, 1, 1], initializer=tf.initializers.he_normal())
                    temp = w1 * p3 + w2 * self.upsample_layer(P_4_td, factor=2., 'p4_td_upsample')
                    P_3_out = conv_bn_relu(temp/(w1 + w2 + EPSILON), W_bifpn, "conv3_out", "conv3_out/BatchNorm")
                with tf.variable_scope("output_p4"):
                    w1 = tf.get_variable("W1", [1, 1, 1, 1], initializer=tf.initializers.he_normal())
                    w2 = tf.get_variable("W2", [1, 1, 1, 1], initializer=tf.initializers.he_normal())
                    w3 = tf.get_variable("W3", [1, 1, 1, 1], initializer=tf.initializers.he_normal())
                    temp = w1 * p4 + w2 * P_4_td + w3 * self.upsample_layer(P_3_out, factor=0.5, 'p3_out_downsample')
                    P_4_out = conv_bn_relu(temp/(w1 + w2 + w3 + EPSILON), W_bifpn, "conv4_out", "conv4_out/BatchNorm")
                with tf.variable_scope("output_p5"):
                    w1 = tf.get_variable("W1", [1, 1, 1, 1], initializer=tf.initializers.he_normal())
                    w2 = tf.get_variable("W2", [1, 1, 1, 1], initializer=tf.initializers.he_normal())
                    w3 = tf.get_variable("W3", [1, 1, 1, 1], initializer=tf.initializers.he_normal())
                    temp = w1 * p5 + w2 * P_5_td + w3 * self.upsample_layer(P_4_out, factor=0.5, 'p4_out_downsample')
                    P_5_out = conv_bn_relu(temp/(w1 + w2 + w3 + EPSILON), W_bifpn, "conv5_out", "conv5_out/BatchNorm")
                with tf.variable_scope("output_p6"):
                    w1 = tf.get_variable("W1", [1, 1, 1, 1], initializer=tf.initializers.he_normal())
                    w2 = tf.get_variable("W2", [1, 1, 1, 1], initializer=tf.initializers.he_normal())
                    w3 = tf.get_variable("W3", [1, 1, 1, 1], initializer=tf.initializers.he_normal())
                    temp = w1 * p6 + w2 * P_6_td + w3 * self.upsample_layer(P_5_out, factor=0.5, 'p5_out_downsample')
                    P_6_out = conv_bn_relu(temp/(w1 + w2 + w3 + EPSILON), W_bifpn, "conv6_out", "conv6_out/BatchNorm")
                with tf.variable_scope("output_p7"):
                    w1 = tf.get_variable("W1", [1, 1, 1, 1], initializer=tf.initializers.he_normal())
                    w2 = tf.get_variable("W2", [1, 1, 1, 1], initializer=tf.initializers.he_normal())
                    temp = w1 * p7 + w2 * self.upsample_layer(P_6_out, factor=0.5, 'p6_out_downsample')
                    P_7_out = conv_bn_relu(temp/(w1 + w2 + w3 + EPSILON), W_bifpn, "conv7_out", "conv7_out/BatchNorm")
...
#     net = gn_(input_ = in_put, is_training = is_training, scope = scope)
    if activation:
        net = tf.nn.relu6(net)
    return net

def conv_bn_relu(x, filters = 64, conv_scope, bn_scope):
    x = slim.conv2d(x, filters, [3, 3], trainable=self.is_training,
                                weights_initializer=tf.truncated_normal_initializer(mean=0.0, stddev=0.03),
                                biases_initializer=None,
                                padding="SAME",
                                stride=1,
                                activation_fn=None,
                                scope=conv_scope)
    x = bn_relu(x, activation, is_training, bn_scope)
    return x
...
#     net = conv_2d_same(in_put_, filters, kernel_size, strides, rate, is_training, scope)
    ...
    net = gn_(input_ = net, is_training = is_training, scope = BN_scope + scope)
    if activation:
        net = tf.nn.relu6(net)
    return net
